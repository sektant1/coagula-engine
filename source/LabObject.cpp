...
LabObject::LabObject()
{
    ...
    // load shaders
    mnd::FileReader vertShader(kCoolVertShaderPath);
    mnd::FileReader fragShader(kGreenFragShaderPath);
    ...
    auto &gfx  = mnd::Engine::GetInstance().GetGraphicsAPI();
    auto  prog = gfx.CreateShaderProgram(vs, fs);
    if (!prog) {
        LOG_ERROR("LabObject failed to create shader program");
    }

    m_material = std::make_shared<mnd::Material>();
    m_material->SetShaderProgram(prog);

    auto mesh = mnd::Builder::CreateFullscreenQuad().buildMesh();
    AddComponent(new mnd::MeshComponent(m_material, mesh));
    LOG_INFO("LabObject constructed");
}

void LabObject::Update(mnd::f32 deltaTime)
{
    mnd::GameObject::Update(deltaTime);

    auto  position = GetPosition();
    auto &input    = mnd::Engine::GetInstance().GetInputManager();
    if (input.IsKeyPressed(mnd::Key::Num1)) {
        m_timeScale = kLabTimeScale1;
    }
    if (input.IsKeyPressed(mnd::Key::Num2)) {
        m_timeScale = kLabTimeScale2;
    }
    if (input.IsKeyPressed(mnd::Key::Num3)) {
        m_timeScale = kLabTimeScale3;
    }
    if (input.IsKeyPressed(mnd::Key::Num4)) {
        m_timeScale = kLabTimeScale4;
    }
    if (input.IsKeyPressed(mnd::Key::Num5)) {
